...
import './AddEncounterModal.css';
import API_BASE_URL from '../../apiConfig';

function AddEncounterModal({ onClose, onAddEncounter }) {
  ...
  const [selectedCharacters, setSelectedCharacters] = useState([]); // This will be pre-populated
  ...
  useEffect(() => {
    ...
    // Fetch characters and pre-select PCs
    const charactersUrl = currentCampaign ? `${API_BASE_URL}/characters?campaignId=${currentCampaign.id}` : `${API_BASE_URL}/characters`;
    fetch(charactersUrl)
      .then(response => response.json())
      .then(data => {
        setAllCharacters(data);
        // Pre-select characters where is_player_character is true
        const pcs = data.filter(char => char.is_player_character);
        setSelectedCharacters(pcs);
      })
      .catch(error => console.error('Error fetching characters:', error));

    // Set current campaign by default
    if (currentCampaign) {
  ...
  const handleSubmit = (e) => {
    ...
    onAddEncounter({
      ...
      characters: selectedCharacters.map(c => c.id), // Send only IDs
    });
  };
          ...
          <div className="form-group difficulty-display">
            ...
            {difficultyData.adjustedXP > 0 && (
              <div className="difficulty-formula">
                ...
                <p><strong>Umbrales del Grupo:</strong></p>
                <ul>
